...
public class DocumentServiceImpl implements DocumentService {
    ...
    public FullDocumentDto get(Long id, String type) {
        ...
        switch (RequestDataType.from(type)
            ...
            case REMARK -> document = repository.findByRemarkId(id);
            case TASK_JOURNAL -> document = repository.findByTaskJournalId(id);
        }
    ...
    public List<FullDocumentDto> getAll(DocumentSearchParam param) {
        ...
        if (param.getEquipmentId() != null && param.getEquipmentId() > 0) {
            booleanBuilder.and(QTasksJournal.tasksJournal.equipmentId.eq(param.getEquipmentId()));
        }
        ...
        if (param.getStartDate() != null && param.getEndDate() != null) {
            booleanBuilder.and(QTasksJournal.tasksJournal.date.after(param.getStartDate()));
            booleanBuilder.and(QTasksJournal.tasksJournal.date.before(param.getEndDate()));
        } else {
